...
class FlexopStructure:
    ...
    def create_elem_indexing(self) -> None:
        ...
        print(self.elem_slice['wing1_main'])
        print(self.elem_slice['wing2_main'])
        if self.include_tail:
    ...
    def create_node_indexing(self) -> None:
        ...
        if self.use_multibody:
            ...
            i_end: int = 3
            print(node_index_cum)
            print(self.node_slice['wing1_main'])
            print(self.node_slice['wing1_tip'])
        else:
            self.node_slice.update({'wing1_inner': np.arange(node_index_cum[0], node_index_cum[1] + 1),
                                    'wing1_main': np.arange(node_index_cum[1], node_index_cum[2] + 1),
...
class FlexopAeroelastic(FlexopStructure):
    def __init__(self, case_name, case_route, **kwargs):
        ...
        if self.include_elevators and self.include_tail:
            ...
            print(self.control_surface_deflection)
    ...
    def generate_elevators(self) -> None:
        ...
        self.control_surface_deflection = [self.input_settings.get('elevator_angle', 0.), self.input_settings.get('elevator_angle', 0.), -self.input_settings.get('aileron_angle', 0.), self.input_settings.get('aileron_angle', 0.), self.input_settings.get('tab_angle_right', 0.), self.input_settings.get('tab_angle_left', 0.)]
        self.control_surface_chord = [self.m_tail // 2, self.m_tail // 2, self.m_wing // 4, self.m_wing // 4, self.m_wing // 4, self.m_wing // 4]
        self.control_surface_hinge_coord = np.zeros(6)
        self.control_surface_type = np.zeros(6, dtype=int)

        self.control_surface[self.elem_slice['tail1'], :] = 0
        self.control_surface[self.elem_slice['tail2'], :] = 1
        self.control_surface[14:18, :] = 2
        self.control_surface[41:45, :] = 3
        self.control_surface[self.elem_slice['wing1_tip'], :] = 4
        self.control_surface[self.elem_slice['wing2_tip'], :] = 5
